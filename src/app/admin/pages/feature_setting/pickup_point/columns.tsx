...
const handleDelete = async (id: number) => {
  ...
  try {
    ...
    if (response.ok) {
      showSuccessToast("Pickup Point deleted successfully");
    } else {
      const errorData = await response.json();
      showErrorToast(errorData.error || "Error deleting Pickup Point");
    }
  } catch (error) {
...
export const columns: ColumnDef<Products>[] = [
      ...
      return (
        <DropdownMenu>
          ...
          <DropdownMenuContent className="dark:bg-slate-700 dark:text-slate-200 bg-slate-100" align="start">
            <DropdownMenuItem className="dark:hover:bg-slate-500 hover:bg-slate-300"><Link href={`/admin/pages/feature_setting/pickup_point/addnew?id=${id}`}>Edit</Link></DropdownMenuItem>
  ...
  {
    accessorKey: "pickup_station_contact",

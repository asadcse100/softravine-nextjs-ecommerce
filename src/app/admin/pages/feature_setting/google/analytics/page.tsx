...
export default function AddOrEdit() {
    ...
    if (id) {
      const fetchTicket = async () => {
        try {
          ...
          const response = await fetch(`${apiUrl}/server/api/routes/admin/feature_setting/google/analytics/${id}`);
  ...
  const onSubmit: SubmitHandler<z.infer<typeof formSchema>> = async (values) => {
    ...
    try {
      ...
      const url = id
        ? `${apiUrl}/server/api/routes/admin/feature_setting/google/analytics/${id}`
        : `${apiUrl}/server/api/routes/admin/feature_setting/google/analytics`;
      ...
      if (!response.ok) {
        throw new Error("Failed to add google analytics. Please try again.");
      }

      const result = await response.json();

      showSuccessToast(result.message || "google analytics added successfully!");
      // router.push("/admin/pages/blog_system/google analytics");
      window.location.href = `${apiUrl}/admin/pages/blog_system/google analytics`;
    } catch (error) {
      showErrorToast("Error adding google analytics: " + (error instanceof Error ? error.message : "Unknown error"));
    }
  };
        ...
        <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8">
          <div className="mx-auto max-w-screen-2xl">
            <div className="mb-3 flex flex-row items-center justify-between gap-3 sm:flex-row sm:items-center sm:justify-between">
              <Breadcrumb pageName="Google Analytics" />
            </div>
                ...
                <div className="px-6 rounded-sm border border-stroke bg-white shadow-default dark:border-strokedark dark:bg-boxdark">
                  <div className="border-b border-stroke px-6.5 py-4 dark:border-strokedark">
                    <h3 className="font-medium text-black dark:text-slate-300">
                      {id ? "Edit Facebook Pixel Setting" : "Add Facebook Pixel Setting"}
                    </h3>
                  </div>
                  <div className="py-6">

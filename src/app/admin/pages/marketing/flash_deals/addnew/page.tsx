...
export default function AddOrEdit() {
    ...
    if (id) {
      const fetchTicket = async () => {
        try {
          ...
          const response = await fetch(`${apiUrl}/server/api/routes/admin/marketing/flash_deals/${id}`);
  ...
  const onSubmit: SubmitHandler<z.infer<typeof formSchema>> = async (values) => {
    ...
    try {
      ...
      const url = id
        ? `${apiUrl}/server/api/routes/admin/marketing/flash_deals/${id}`
        : `${apiUrl}/server/api/routes/admin/marketing/flash_deals`;
  ...
  const [products, setproducts] = useState<{ id: string; name: string }[]>([]) // Adjust the type according to your data structure
  // Fetch data from an API
  useEffect(() => {
    const fetchproducts = async () => {
      const apiUrl = process.env.NEXT_PUBLIC_API_URL || ''
      try {
        const response = await fetch(`${apiUrl}/server/api/routes/admin/marketing/select/products`) // Replace with your API endpoint
        const data = await response.json()
                ...
                <div className="px-6 rounded-sm border border-stroke bg-white shadow-default dark:border-strokedark dark:bg-boxdark">
                  <div className="border-b border-stroke px-6.5 py-4 dark:border-strokedark">
                    <h3 className="font-medium text-black dark:text-white">
                      {id ? "Edit Flash Deal Information" : "Add Flash Deal Information"}
                    </h3>
                  </div>
                  <div className="py-6">

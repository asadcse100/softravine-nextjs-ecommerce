...
export default function Addnew() {
  ...
  const onSubmit: SubmitHandler<z.infer<typeof formSchema>> = async (values) => {
    ...
    try {
      const response = await fetch(`${apiUrl}/server/api/routes/admin/marketing/pop_up`, {
        method: "POST",
        headers: {
      ...
      if (!response.ok) {
        throw new Error("Failed to add pop up. Please try again.");
      }

      const result = await response.json();

      showSuccessToast(result.message || "pop up added successfully!");
      // router.push("/admin/pages/blog_system/pop up");
      window.location.href = `${apiUrl}/admin/pages/blog_system/pop up`;
    } catch (error) {
      showErrorToast("Error adding pop up: " + (error instanceof Error ? error.message : "Unknown error"));
    }
  };

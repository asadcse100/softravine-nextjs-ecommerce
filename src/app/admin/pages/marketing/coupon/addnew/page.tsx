...
export default function AddOrEdit() {
    ...
    if (id) {
      const fetchTicket = async () => {
        try {
          ...
          const response = await fetch(`${apiUrl}/server/api/routes/admin/marketing/coupon/${id}`);
  ...
  const onSubmit: SubmitHandler<z.infer<typeof formSchema>> = async (values) => {
    ...
    try {
      ...
      const url = id
        ? `${apiUrl}/server/api/routes/admin/marketing/coupon/${id}`
        : `${apiUrl}/server/api/routes/admin/marketing/coupon`;
      ...
      showSuccessToast(result.message || "coupon added successfully!");
      // router.push("/admin/pages/blog_system/coupon");
      window.location.href = `${apiUrl}/admin/pages/blog_system/coupon`;
    } catch (error) {
      showErrorToast("Error adding coupon: " + (error instanceof Error ? error.message : "Unknown error"));
    }
  };
                ...
                <div className="px-6 rounded-sm border border-stroke bg-white shadow-default dark:border-strokedark dark:bg-boxdark">
                  <div className="border-b border-stroke px-6.5 py-4 dark:border-strokedark">
                    <h3 className="font-medium text-black dark:text-white">
                      {id ? "Edit Coupon Information Adding" : "Add Coupon Information Adding"}
                    </h3>
                  </div>
                  <div className="py-6">

...
export default function AddOrEdit() {
    ...
    if (id) {
      const fetchTicket = async () => {
        try {
          ...
          const response = await fetch(`${apiUrl}/server/api/routes/admin/setting/page/${id}`);
  ...
  const onSubmit: SubmitHandler<z.infer<typeof formSchema>> = async (values) => {
    ...
    try {
      ...
      const url = id
        ? `${apiUrl}/server/api/routes/admin/setting/page/${id}`
        : `${apiUrl}/server/api/routes/admin/setting/page`;
      ...
      if (!response.ok) {
        throw new Error("Failed to add custome page. Please try again.");
      }

      const result = await response.json();

      showSuccessToast(result.message || "custome page added successfully!");
      // router.push("/admin/pages/blog_system/custome page");
      window.location.href = `${apiUrl}/admin/pages/setting/page`;
    } catch (error) {
      showErrorToast("Error adding custome page: " + (error instanceof Error ? error.message : "Unknown error"));
    }
  };
        ...
        <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8">
          <div className="mx-auto max-w-screen-2xl">
            <div className="mb-3 flex flex-row items-center justify-between gap-3 sm:flex-row sm:items-center sm:justify-between">
              <Breadcrumb pageName="Website Page Add" />
            </div>
                ...
                <div className="px-6 rounded-sm border border-stroke bg-white shadow-default dark:border-strokedark dark:bg-boxdark">
                  <div className="border-b border-stroke px-6.5 py-4 dark:border-strokedark">
                    <h3 className="font-medium text-black dark:text-white">
                      {id ? "Edit Page Content" : "Add Page Content"}
                    </h3>
                  </div>
                  <div className="py-6">
                    ...
                    {[
                        ...
                        { name: "content", label: "Add Content"},
                      ].map((field) => (
                        <div
                          key={field.name}

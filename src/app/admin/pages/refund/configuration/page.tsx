...
const formSchema = z.object({
  refund_request_time: z.string().min(1, {
    message: "Refund Request Time required!",
  }),
});
...
export default function AddOrEdit() {
    ...
    if (id) {
      const fetchTicket = async () => {
        try {
          ...
          const response = await fetch(`${apiUrl}/server/api/routes/admin/refund/configuration/${id}`);
  ...
  const onSubmit: SubmitHandler<z.infer<typeof formSchema>> = async (values) => {
    ...
    try {
      ...
      const url = id
        ? `${apiUrl}/server/api/routes/admin/refund/configuration/${id}`
        : `${apiUrl}/server/api/routes/admin/refund/configuration`;
      ...
      <Form {...form}>
        <div className="mx-auto max-w-screen-2xl">
          <div className="mb-3 flex flex-row items-center justify-between gap-3 sm:flex-row sm:items-center sm:justify-between">
            <Breadcrumb pageName="Refund Configuration" />
          </div>
                ...
                <div className="px-6 rounded-sm border border-stroke bg-white shadow-default dark:border-strokedark dark:bg-boxdark">
                  <div className="border-b border-stroke px-6.5 py-4 dark:border-strokedark">
                    <h3 className="font-medium text-black dark:text-white">
                      {id ? "Edit Set Refund Time" : "Add Set Refund Time"}
                    </h3>
                  </div>
                  <div className="py-6">
                          ...
                          <FormItem>
                            <div className="grid grid-cols-1 md:grid-cols-12">
                              <div className="col-span-3 mt-2">
                                <FormLabel>Refund Request Time</FormLabel>
                              </div>

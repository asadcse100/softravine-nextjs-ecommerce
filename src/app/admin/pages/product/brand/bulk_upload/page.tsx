...
export default function AddOrEdit() {
    ...
    if (id) {
      const fetchTicket = async () => {
        try {
          ...
          const response = await fetch(`${apiUrl}/server/api/routes/admin/product/bulk_upload/${id}`);
  ...
  const onSubmit: SubmitHandler<z.infer<typeof formSchema>> = async (values) => {
    ...
    try {
      ...
      const url = id
        ? `${apiUrl}/server/api/routes/admin/product/bulk_upload/${id}`
        : `${apiUrl}/server/api/routes/admin/product/bulk_upload`;
      ...
      if (!response.ok) {
        throw new Error("Failed to add bulk upload. Please try again.");
      }

      const result = await response.json();

      showSuccessToast(result.message || "bulk upload added successfully!");
      // router.push("/admin/pages/blog_system/bulk upload");
      window.location.href = `${apiUrl}/admin/pages/blog_system/bulk upload`;
    } catch (error) {
      showErrorToast("Error adding bulk upload: " + (error instanceof Error ? error.message : "Unknown error"));
    }
  };

...
export default function AddOrEdit() {
  ...
  const onSubmit: SubmitHandler<z.infer<typeof formSchema>> = async (values) => {
    ...
    try {
      ...
      const url = id
        ? `${apiUrl}/server/api/routes/admin/otp/cerdential/${id}`
        : `${apiUrl}/server/api/routes/admin/otp/cerdential`;
      ...
      if (!response.ok) {
        throw new Error("Failed to add otp cerdential. Please try again.");
      }

      const result = await response.json();

      showSuccessToast(result.message || "otp cerdential added successfully!");
      // router.push("/admin/pages/blog_system/otp cerdential");
      window.location.href = `${apiUrl}/admin/pages/blog_system/otp cerdential`;
    } catch (error) {
      showErrorToast("Error adding otp cerdential: " + (error instanceof Error ? error.message : "Unknown error"));
    }
  };
        ...
        <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8">
          <div className="mx-auto max-w-screen-2xl">
            <div className="mb-3 flex flex-row items-center justify-between gap-3 sm:flex-row sm:items-center sm:justify-between">
              <Breadcrumb pageName="OTP Configuration Credentials" />
            </div>
                ...
                <div className="px-6 rounded-sm border border-stroke bg-white shadow-default dark:border-strokedark dark:bg-boxdark">
                  <div className="border-b border-stroke px-6.5 py-4 dark:border-strokedark">
                    <h3 className="font-medium text-black dark:text-white">
                      {id ? "Edit Nexmo Credential" : "Add Nexmo Credential"}
                    </h3>
                  </div>
                  <div className="py-6">

...
export const storeAffiliateUser = async (req: NextRequest) => {
  ...
  const affiliate_users = await prisma.affiliate_users.update({
    where: { userId },
...
export const deleteAffiliateUser = async (id: number) => {
  try {
    // Check if the record exists
    const existingAirTickets = await prisma.affiliate_users.findUnique({
      where: { id },
    });
    ...
    const deletedAirTickets = await prisma.affiliate_users.delete({
      where: { id },
    });

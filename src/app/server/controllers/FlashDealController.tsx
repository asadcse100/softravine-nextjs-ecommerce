...
export const getFlashDealById = async (id: number) => {
  try {
    // Check if the record exists
    const existingCategory = await prisma.flash_deals.findUnique({
      where: { id },
    });
...
export const createOrUpdateFlashDeal = async (req: FlashDealRequest) => {
...
export const deleteFlashDeal = async (id: number) => {
  ...
  const deleteFlashDeal = await prisma.flash_deals.delete({ where: { id } });

  // return 'Flash Deal has been deleted successfully';
  return { success: true, data: deleteFlashDeal };
};
...
export const updateFlashDealStatus = async (id: number, status: number) => {
  ...
  return { success: true, data: updatedFlashDeal };
  // return updatedFlashDeal;
};

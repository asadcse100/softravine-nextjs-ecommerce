...
export default function AccountNomineePage() {
    ...
    if (id) {
      const fetchTicket = async () => {
        try {
          ...
          const response = await fetch(`${apiUrl}/server/api/routes/business/nominee/${id}`);
  ...
  const onSubmit: SubmitHandler<z.infer<typeof formSchema>> = async (values) => {
    ...
    try {
      ...
      const url = id
        ? `${apiUrl}/server/api/routes/business/nominee/${id}`
        : `${apiUrl}/server/api/routes/business/nominee`;

...
export default function AddOrEdit() {
    ...
    if (id) {
      const fetchTicket = async () => {
        try {
          ...
          const response = await fetch(`${apiUrl}/server/api/routes/business/wholesale/${id}`);
  ...
  const onSubmit: SubmitHandler<z.infer<typeof formSchema>> = async (values) => {
    ...
    try {
      ...
      const url = id
        ? `${apiUrl}/server/api/routes/business/wholesale/${id}`
        : `${apiUrl}/server/api/routes/business/wholesale`;
          ...
          <div className="mx-auto max-w-screen-2xl">
            <div className="border-b border-stroke dark:border-strokedark">
              <h3 className="font-medium text-black dark:text-white">
                {id ? "Edit Your Wholesale Product" : "Add Your Wholesale Product"}
              </h3>
            </div>
